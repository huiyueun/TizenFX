...
namespace Tizen.NUI
{
    ...
    public abstract class FrameBrokerBase : IDisposable
    {
        ...
        public FrameBrokerBase(Window window)
        {
            ...
            //err = Interop.FrameBroker.Create(Any.getCPtr(window.GetNativeHandle()).Handle, ref _callbacks, IntPtr.Zero, out _handle);
            int id = window.GetNativeId();
            err = Interop.FrameBroker.Create(Interop.EcoreWl2.FindWindow((uint)id), ref _callbacks, IntPtr.Zero, out _handle);
